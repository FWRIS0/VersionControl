...
namespace UnitTestExample.Controllers
{
    ...
    {        
        ...
        {
            Regex kis = new Regex(@"[a-z]");
            Regex nagy = new Regex(@"[A-Z]");
            Regex num = new Regex(@"[0-9]");
            Regex hossz = new Regex(@"[a-zA-Z0-9]{8,}");
            if (kis.IsMatch(password) && nagy.IsMatch(password) && num.IsMatch(password) && hossz.IsMatch(password) )
            {
                return true;
            }
            return false;
